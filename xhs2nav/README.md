...
高德/百度得到的是**逐段导航**：N 个地点拆成 N-1 段，每段 A→B 用地名直传，每到一站回来点下一段。

**iOS 上还会多给一条 Apple 地图链接，一条走完全程**——Apple 的 `waypoint` 参数
官方明文接受纯地名，是三家里唯一不需要坐标的。
...
**为什么高德/百度必须要坐标**：它们的 `vialons`/`vialats` 和 `viaPoints` 都把经纬度写成必填字段。
