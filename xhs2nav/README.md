...
1. 截图里长按文字 → 复制（iOS 实况文本 / 小米「识别文字」，不需要任何 App）
2. 粘贴进来——地点自动出现，不用点按钮；改名/排序/删除随手做
3. 点「出发」

坐标你不用管：出发时工具自动在背后定位（默认免 Key 走 OpenStreetMap；
设置里填了地图 Key 会自动改用高德/百度，更准）。定位失败也不挡路——自动退成逐段导航。
iOS 上还会多给一条 Apple 地图链接：它的途经点官方明文接受纯地名，本来就不需要坐标。

选截图直读（多模态模型识别）是可选项，需要在设置里配模型 Key。
