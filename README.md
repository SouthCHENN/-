...
- **票根上传与展示**（原图存 IndexedDB，离线持久；网格渲染 320px 缩略图，避免大图解码阻塞主线程）：节点详情卡「本节点票根」区直接添加（壳层 onShowFileChooser 拉起相册/文件选择器）；行程页「本日票根」卡聚合当天全部节点（缩略图带节点名角标），添加时先选所属节点；长按缩略图删除（二次确认）；点击全屏查看原图，下拉手势关闭
...
./build.sh          # 产物: dist/zouzhe-v1.2.2.apk
```
