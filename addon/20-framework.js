...
(function () {
  ...
  var CSS = [
    ...
    '.zz-tbtheme{flex:none;width:34px;height:34px;margin-left:2px;display:flex;align-items:center;justify-content:center;',
    'background:rgba(0,240,255,.05);border:1px solid rgba(0,240,255,.35);border-radius:3px;color:#00F0FF;cursor:pointer;',
    'padding:0;-webkit-tap-highlight-color:transparent}',
    '.zz-tbtheme svg{width:19px;height:19px;display:block}',
    /* overlay */
  ...
  function boot() {
    ...
    applyTheme(getTheme());
    /* 主题按钮点击走文档级捕获委托：App 克隆重建按钮也不失效 */
    document.addEventListener('click', function (e) {
      if (e.target && e.target.closest && e.target.closest('.zz-tbtheme')) {
        e.preventDefault();
        e.stopPropagation();
