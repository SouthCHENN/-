...
(function () {
  ...
  var CSS = [
    ...
    /* C：标题旁徽章 + 独立弹层 */
    '.zz-wcchip{display:inline-flex;align-items:center;gap:4px;margin:6px 0 0;padding:4px 10px;min-height:30px;',
    'border:1px solid rgba(0,240,255,.35);border-radius:3px;background:rgba(0,240,255,.05);',
    'font:600 10.5px "Share Tech Mono",monospace;color:#00F0FF;cursor:pointer;-webkit-tap-highlight-color:transparent}',
    '.zz-wcchip.warn{border-color:rgba(255,46,136,.5);background:rgba(255,46,136,.07);color:#FF2E88}',
    '.zz-wcpop{position:fixed;inset:0;z-index:9500}',
    '.zz-wcpop-scrim{position:absolute;inset:0;background:rgba(2,4,10,.6)}',
    '.zz-wcpop-card{position:absolute;left:16px;right:16px;bottom:calc(90px + env(safe-area-inset-bottom));max-width:480px;margin:0 auto;',
    'background:#0D1424;border:1px solid rgba(0,240,255,.4);border-radius:6px;padding:14px 16px;box-shadow:0 0 30px rgba(0,240,255,.15)}',
    '.zz-wcpop-card.warn{border-color:rgba(255,46,136,.5);box-shadow:0 0 30px rgba(255,46,136,.15)}',
    '.zz-wcpop-card b{display:block;font:600 11px "Share Tech Mono",monospace;color:#00F0FF;letter-spacing:.5px;margin-bottom:2px}',
    '.zz-wcpop-card.warn b{color:#FF2E88}',
    '.zz-wcpop-card i{display:block;font:italic 400 11px "Noto Sans SC",system-ui;color:#5E7186;margin-bottom:7px;font-style:normal}',
    '.zz-wcpop-card p{margin:0;font:13px/1.75 "Noto Sans SC",system-ui;color:#D8E6F0}',
    '.zz-wcpop-x{display:block;width:100%;min-height:40px;margin-top:12px;border:1px solid rgba(0,240,255,.35);border-radius:3px;',
    'background:transparent;color:#00F0FF;font:600 13px "Noto Sans SC",system-ui;cursor:pointer}',
  ...
  /* ---------------- 节点详情卡 · 厕所位置（独立区块，三种交互方案） ----------------
     方案由 window.__ZZ_WC_MODE 控制：'A'=卡尾折叠抽屉 'B'=卡尾分区直展 'C'=徽章+独立弹层 */
  var wcCur = null; /* 当前节点命中的厕所信息 {key,text,warn,title} */

  function ensureWc() {
    var WC = window.__ZZ_WC || {};
    var mode = window.__ZZ_WC_MODE || 'A';
    ...
    var titleEl = null, title = '';
    var inner = sheet.getElementsByTagName('div');
    for (var j = 0; j < inner.length; j++) {
      var s2 = inner[j].getAttribute('style') || '';
      if (s2.indexOf('17px') >= 0) { titleEl = inner[j]; title = (inner[j].textContent || '').trim(); break; }
    }
    function clearAll() {
      var olds = sheet.querySelectorAll('.zz-wcsec,.zz-wcchip');
      for (var o = 0; o < olds.length; o++) olds[o].parentElement.removeChild(olds[o]);
    }
    if (!dm || !title || !titleEl) { clearAll(); wcCur = null; return; }
    ...
    var key = mode + '|' + day + '|' + title;
    var exist = sheet.querySelector('.zz-wcsec,.zz-wcchip');
    if (exist && exist.getAttribute('data-key') === key) return;
    clearAll();
    if (text == null) { wcCur = null; return; }
    var warn = text.indexOf('⚠️') >= 0;
    wcCur = { key: key, text: text, warn: warn, title: title };
    var lines = text.split('\n').map(esc).join('<br>');

    if (mode === 'C') {
      var chip = document.createElement('div');
      chip.className = 'zz-wcchip' + (warn ? ' warn' : '');
      chip.setAttribute('data-key', key);
      chip.innerHTML = '🚻 厕所位置' + (warn ? ' ⚠️' : '') + ' ›';
    ...
    var open = mode === 'B';
    sec.innerHTML =
      '<div class="zz-wch">🚻 WC// 厕所位置' +
      (mode === 'A' ? '<span class="zz-wcarw">' + (open ? '▾' : '▸') + '</span>' : '') +
      '</div>' +
      '<div class="zz-wcb"' + (open ? '' : ' style="display:none"') + '>' + lines + '</div>';
    sheet.appendChild(sec); /* 卡尾：位于按钮与离线说明之后，独立分区 */
  }

  function wcPopupOpen() {
    if (!wcCur) return;
    wcPopupClose();
    var pop = document.createElement('div');
    pop.className = 'zz-wcpop';
    pop.innerHTML =
      '<div class="zz-wcpop-scrim"></div>' +
      '<div class="zz-wcpop-card' + (wcCur.warn ? ' warn' : '') + '">' +
      '<b>🚻 WC// 厕所位置</b><i>' + esc(wcCur.title) + '</i>' +
      '<p>' + wcCur.text.split('\n').map(esc).join('<br>') + '</p>' +
      '<button class="zz-wcpop-x">关闭</button></div>';
    document.body.appendChild(pop);
  }
  function wcPopupClose() {
    var p = document.querySelector('.zz-wcpop');
  ...
  function boot() {
    ...
    document.addEventListener('click', function (e) {
      ...
      var wch = t.closest('.zz-wch');
      if (wch && wch.parentElement && wch.parentElement.classList.contains('zz-wcsec')) {
        /* 方案A：折叠抽屉开合 */
        if ((window.__ZZ_WC_MODE || 'A') === 'A') {
          e.stopPropagation();
          var body = wch.parentElement.querySelector('.zz-wcb');
          var arw = wch.querySelector('.zz-wcarw');
          var vis = body && body.style.display !== 'none';
          if (body) body.style.display = vis ? 'none' : '';
          if (arw) arw.textContent = vis ? '▸' : '▾';
        }
        return;
      }
      if (t.closest('.zz-wcchip')) { e.stopPropagation(); wcPopupOpen(); return; }
      if (t.closest('.zz-wcpop-x') || (t.closest('.zz-wcpop-scrim'))) { e.stopPropagation(); wcPopupClose(); return; }
    }, true);
