...
(function () {
  ...
  var CSS = [
    ...
    /* 行程页内嵌地图预览（并入 ROUTE 路线卡同一框内，divider 分隔） */
    '.zz-inline{margin:8px 4px 6px;padding-top:2px;border-top:1px solid rgba(0,240,255,.15);background:transparent;cursor:pointer;-webkit-tap-highlight-color:transparent}',
    '.zz-inhead{display:flex;justify-content:space-between;align-items:baseline;gap:8px;padding:8px 4px 7px}',
  ...
  function ensureInline() {
    ...
    var ok = inlineBox && inlineBox.isConnected &&
      inlineBox.parentElement === r.card && inlineState.day === r.day;
    if (ok) return;
