...
(function () {
  ...
  var CSS = [
    ...
    /* 行程页内嵌地图预览卡（挂在 ROUTE 路线卡下方） */
    '.zz-inline{margin:10px 14px 0;border-radius:4px;border:1px solid rgba(0,240,255,.22);background:rgba(0,240,255,.04);overflow:hidden;cursor:pointer;-webkit-tap-highlight-color:transparent}',
    '.zz-inhead{display:flex;justify-content:space-between;align-items:baseline;gap:8px;padding:9px 12px 7px}',
    ...
    '.zz-inthumb{position:relative;border-top:1px solid rgba(0,240,255,.15)}',
    ...
    '.zz-intabs{display:flex;gap:6px;padding:8px 10px;border-top:1px solid rgba(0,240,255,.15);flex-wrap:wrap}',
  ...
  function findRouteCard() {
    ...
    for (var i = 0; i < divs.length; i++) {
      ...
      if (card) return { day: +mm[1], card: card };
    }
    return null;
  }
  ...
  function ensureInline() {
    ...
    var ok = inlineBox && inlineBox.isConnected &&
      inlineBox.previousElementSibling === r.card && inlineState.day === r.day;
    if (ok) return;
    ...
    renderInline(r.day);
    r.card.insertAdjacentElement('afterend', inlineBox);
  }
